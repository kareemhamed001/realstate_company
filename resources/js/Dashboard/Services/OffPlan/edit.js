...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!inputElement.id === 'project_cover_image') {


        inputElement.forEach((inputElement) => {
            let imgElement = inputElement.parentElement.querySelector('img');
            if (imgElement) {
...
function onChangeImagePreview(imgPreviewId, inputId) {
    let inputElement = document.getElementById(inputId);
    let imgElement = document.getElementById(imgPreviewId);
    inputElement.addEventListener('change', (e) => {
        let file = e.target.files[0];
        let reader = new FileReader();
        reader.onloadend = function () {
...
let lastAmenityElementChild = amenityContainer.lastElementChild;
let amenityCount = 0;
if (lastAmenityElementChild) {
    //get the last amenity count as integer
    if (lastAmenityElementChild.dataset.count)
        amenityCount = parseInt(lastAmenityElementChild.dataset.count) + 1;
}
...
if (addAmenityBtn) {
    addAmenityBtn.addEventListener('click', (e) => {
                        ...
                        <div class="card-body p-2 border-0">
                            <label class="service-image-container border-0" for="features[${amenityCount}][image]">
                                <img class="img-fluid service-image border-0" src=""
                                     alt="" id="featureImgPreview-${amenityCount}">
                            </label>
        ...
        `;
            ...
            onChangeImagePreview(`featureImgPreview-${amenityCount}`, `features[${amenityCount}][image]`)
...
let lastGalleryImageElementChild = galleryImageContainer.lastElementChild;
let galleryImageCount = 0;
if (lastGalleryImageElementChild) {
    //get the last gallery Image count as integer
    if (lastGalleryImageElementChild.dataset.count)
        galleryImageCount = parseInt(lastGalleryImageElementChild.dataset.count) + 1;
}
if (addGalleryImageBtn) {
  ...
  <label for="images[${galleryImageCount}]"
        ...
        `;
            ...
            onChangeImagePreview(`galleryImagePreview-${galleryImageCount}`, `images[${galleryImageCount}]`)
...
let lastPhotoImageElementChild = photoContainer.lastElementChild;
let photoImageCount = 0;
if (lastPhotoImageElementChild) {
    //get the last photo Image count as integer
    if (lastPhotoImageElementChild.dataset.count)
        photoImageCount = parseInt(lastPhotoImageElementChild.dataset.count) + 1;
}
if (addPhotoImageBtn) {
...
let lastPriceElementChild = pricesTableBody.lastElementChild;
let priceCount = 0;
if (lastPriceElementChild) {
    //get the last price count as integer
    if (lastPriceElementChild.dataset.count)
        priceCount = parseInt(lastPriceElementChild.dataset.count) + 1;
}
if (addPriceBtn) {
...
let lastPlanElementChild = plansTimeLine.lastElementChild;
let planCount = 0;
if (lastPlanElementChild) {
    //get the last plan count as integer
    if (lastPlanElementChild.dataset.count)
        planCount = parseInt(lastPlanElementChild.dataset.count) + 1;
}
if (addPlanBtn) {
...
let lastNearPlaceElementChild = nearPlacesContainer.lastElementChild;
let nearPlaceCount = 0;
if (lastNearPlaceElementChild) {
    //get the last near place count as integer
    if (lastNearPlaceElementChild.dataset.count) {
        nearPlaceCount = parseInt(lastNearPlaceElementChild.dataset.count) + 1;
    }
}
...
<i class="fa fa-x text-white"></i>
 ...
 <label class="form-label fw-600 text-primary" for="near_places[${nearPlaceCount}][distance]">distance</label>
                                        <input type="text" class="form-control" name="near_places[${nearPlaceCount}][distance]" value="${distance}" id="near_places[${nearPlaceCount}][distance]" placeholder="distance in km ex:10">
...
<label class="form-label fw-600 text-primary" for="near_places[${nearPlaceCount}][time]">time</label>
                                        ...
                                        <input type="text" class="form-control" name="near_places[${nearPlaceCount}][time]" value="${time}" id="near_places[${nearPlaceCount}][time]" placeholder="time in minutes ex:30">

</div>
<div class="my-2">
<label class="form-label fw-600 text-primary" for="near_places[${nearPlaceCount}][place]">place</label>

                                        <input type="text" class="form-control" name="near_places[${nearPlaceCount}][place]" value="${place}" id="near_places[${nearPlaceCount}][place]" placeholder="place name">
</div>
...
if (editProjectForm) {
    editProjectForm.addEventListener('submit', async (e) => {
        ...
        let projectId = editProjectForm.dataset.projectId;
        ...
        try {
            let response = await offPlan.update(projectId);
